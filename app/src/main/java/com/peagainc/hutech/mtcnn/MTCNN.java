package com.peagainc.hutech.mtcnn;
...
public class MTCNN {
    ...
    public MTCNN(AssetManager assetManager) throws IOException {
        ...
        pInterpreter = new Interpreter(MobileFaceNetUtil.loadModelFile(assetManager, MODEL_FILE_PNET), options);
        rInterpreter = new Interpreter(MobileFaceNetUtil.loadModelFile(assetManager, MODEL_FILE_RNET), options);
        oInterpreter = new Interpreter(MobileFaceNetUtil.loadModelFile(assetManager, MODEL_FILE_ONET), options);
    }
    ...
    private Vector<Box> pNet(Bitmap bitmap, int minSize) {
        ...
        while (currentFaceSize <= whMin) {
            ...
            // (1)Image Resize
            Bitmap bm = MobileFaceNetUtil.bitmapResize(bitmap, scale);
            int w = bm.getWidth();
            int h = bm.getHeight();
            ...
            prob1 = MobileFaceNetUtil.transposeBatch(prob1);
            conv4_2_BiasAdd = MobileFaceNetUtil.transposeBatch(conv4_2_BiasAdd);
    ...
    private void pNetForward(Bitmap bitmap, float[][][][] prob1, float[][][][] conv4_2_BiasAdd) {
        float[][][] img = MobileFaceNetUtil.normalizeImage(bitmap);
        float[][][][] pNetIn = new float[1][][][];
        pNetIn[0] = img;
        pNetIn = MobileFaceNetUtil.transposeBatch(pNetIn);
    ...
    private Vector<Box> rNet(Bitmap bitmap, Vector<Box> boxes) {
        ...
        for (int i = 0; i < num; i++) {
            float[][][] curCrop = MobileFaceNetUtil.cropAndResize(bitmap, boxes.get(i), 24);
            curCrop = MobileFaceNetUtil.transposeImage(curCrop);
            rNetIn[i] = curCrop;
        }
    ...
    private Vector<Box> oNet(Bitmap bitmap, Vector<Box> boxes) {
        ...
        for (int i = 0; i < num; i++) {
            float[][][] curCrop = MobileFaceNetUtil.cropAndResize(bitmap, boxes.get(i), 48);
            curCrop = MobileFaceNetUtil.transposeImage(curCrop);
            oNetIn[i] = curCrop;
        }
