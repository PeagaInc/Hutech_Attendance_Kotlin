package com.peagainc.hutech.mobilefacenet;
...
public class MobileFaceNet {
    ...
    public MobileFaceNet(AssetManager assetManager) throws IOException {
        ...
        interpreter = new Interpreter(MobileFaceNetUtil.loadModelFile(assetManager, MODEL_FILE), options);
    }
    ...
    public float compare(Bitmap bitmap1, Bitmap bitmap2) {
        ...
        MobileFaceNetUtil.l2Normalize(embeddings, 1e-10);
    ...
    private float[][][][] getTwoImageDatasets(Bitmap bitmap1, Bitmap bitmap2) {
        ...
        for (int i = 0; i < ddims[0]; i++) {
            ...
            datasets[i] = MobileFaceNetUtil.normalizeImage(bitmap);
        }
        return datasets;
    }
}
